...
# Define range for diseased area (for example, brown or black spots)

lower_disease = np.array([0, 0, 0])  # Adjust this range according to the color of the disease
upper_disease = np.array([360, 255, 50])

diseasemask = cv2.inRange(hsv, lower_disease, upper_disease)
cv2.imshow('Disease_Mask:', diseasemask)
cnt_d = 0
for d in diseasemask:
    cnt_d = cnt_d + list(d).count(255)
print("Diseased area ", cnt_d)
...
# Check for disease
if cnt_d > threshold_for_disease:
